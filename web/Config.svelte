...
{#if cfg}
  <form onsubmit={submit} class="grid gap-[18px]">
    ...
    <fieldset class={panel}>
      <legend class={legend}>AI scoring</legend>
      <label class="mt-3 grid gap-[7px]">
        <span class={labelSpan}>AI criteria</span>
        <textarea bind:value={cfg.aiCriteria} rows="4" placeholder="Describe the ideal apartment…" class="{control} resize-y leading-normal"></textarea>
      </label>
      <div class="mt-3 grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-[14px]">
    ...
    <fieldset class={panel}>
      <legend class={legend}>Performance</legend>
      <div class="mt-3 grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] gap-[14px]">
...
{:else}
  <div class="glass rounded-[var(--radius-glass)] p-10 text-center text-ink-3">Loading configuration…</div>
{/if}
